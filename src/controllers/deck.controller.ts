 ...
 * @throws {500} Erreur serveur
 */
export const getDeckById = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  try {
    const userId = req.user?.userId;
 ...
 * @throws {500} Erreur serveur
 */
export const deleteDeck = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
  try {
    const userId = req.user?.userId;
    const deckId = parseInt(req.params.id);
